...
MPD Builder（领域适配器）
   把 B 站"非标 DASH JSON"转成标准 MPD XML
       │
       ▼
dash.js  （MSE 引擎）  +  xgplayer / ArtPlayer fallback（UI + 视频状态机）
       │
       ▼
<video> 播出来 🎬
```
...
| v0.6 | **xgplayer 默认 UI** + ArtPlayer 回退 + 插帧播放入口 | 更顺手的控制体验 / 外部 mpv+SVP 补帧 |
...
| 角色 | 选型 | 说明 |
|---|---|---|
| 播放器 UI + 视频状态机 | **xgplayer** 3.x | 默认播放器 UI |
| 回退播放器 | **ArtPlayer** 5.4.1 | 保留弹幕/字幕高级路径与兜底 |
| DASH MSE 引擎 | **dash.js** 5.x | 不可替代 |
| 清晰度/音轨菜单 | **artplayer-plugin-dash-control** | UI 控件层 |
| 弹幕 | xgplayer 轻量弹幕层 / **artplayer-plugin-danmuku** | 默认轻量弹幕，旧播放器保留高级插件 |
...
| **播放器** | `xgplayer` / `ArtPlayer` | 默认 xgplayer；初始化失败会回退 ArtPlayer |
| **显示插帧播放入口** | 开关 | 生成 mpv/SVP 外部播放命令或下载 MPD |
| **清晰度 / 编码 / 音轨** | 默认值 | 播放时可临时切换 |

### 插帧播放

浏览器扩展不能直接启动本机 `mpv` 或 SVP，除非额外安装 Native Messaging
或注册自定义协议。本项目先提供一个稳定的外部入口：

- MP4 资源：点击播放器右上角“插帧”，复制 `mpv --profile=svp "URL"` 命令。
- DASH 资源：点击“插帧”，下载当前集 MPD 文件，并复制 `mpv --profile=svp "文件名.mpd"` 模板。

要真正实时补帧，需要本机安装 `mpv` + SVP / svp4mpv，并用复制出的命令打开。
...
│   │   │   └── mountPlayer.mjs      # xgplayer + dash.js / ArtPlayer fallback
│   │   └── subtitle/
...
│   ├── popup/                 # 工具栏弹窗
│   └── options/               # 高级配置页
├── vendor/                    # 第三方 bundle（xgplayer / ArtPlayer / dash.js / 弹幕）
├── assets/                    # 图标
├── userscripts/               # 配套用户脚本
...
- [xgplayer](https://github.com/bytedance/xgplayer) —— 默认播放器 UI
